...
try:
    ...
    call(
        "if [ -d tmp ]; then rm -rf tmp; fi; mkdir -p tmp/jvm; cd tmp/jvm; tar xvf {}".format(
            filename
        ),
        shell=True,
    )
...
try:
    filename, _ = urllib.request.urlretrieve(
        "https://s3-us-west-2.amazonaws.com/xgboost-docs/doxygen/{}.tar.bz2".format(
            git_branch[0]
        )
    )
    call(
        "mkdir -p tmp/dev; cd tmp/dev; tar xvf {}; mv doc_doxygen/html/* .; rm -rf doc_doxygen".format(
            filename
        ),
        shell=True,
    )
